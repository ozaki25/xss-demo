...
### リセット・運用メモ
- **チャットの投稿を消す**: チャット画面右上の「🗑 投稿を全消去」、または講師用 `/stolen` の
  「チャットの投稿を全消去」ボタン。攻撃メッセージを消すと `<img onerror>` の再発火が止まり、
  **情報窃取も止まります**（次のデモを仕切り直すときに使う）。
- **盗まれた情報を消す**: `/stolen` の「盗まれた情報をリセット」ボタン。
- `/stolen` は**同じ内容の窃取データを1件にまとめて**表示します。`<img onerror>` は再描画のたびに
  発火するため、重複排除しないと同一 Cookie が無限に増えてしまうためです（件数＝被害人数の目安）。
