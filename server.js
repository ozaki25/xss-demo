...
app.get('/messages', (req, res) => {
  const rows = db.prepare('SELECT name, body FROM messages ORDER BY id ASC').all();
  res.json(rows);
});
...
app.post('/messages', (req, res) => {
  ...
  db.prepare('INSERT INTO messages (name, body) VALUES (?, ?)').run(name, body);
  res.json({ ok: true });
});
...
app.get('/steal', (req, res) => {
  ...
  db.prepare('INSERT INTO stolen (data, created_at) VALUES (?, ?)')
...
app.get('/stolen', (req, res) => {
  const rows = db.prepare('SELECT data, created_at FROM stolen ORDER BY id DESC').all();
...
app.post('/reset', (req, res) => {
  db.prepare('DELETE FROM stolen').run();
  res.redirect('/stolen');
});
